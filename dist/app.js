...
document.querySelector('#work-list').innerHTML = work.map((w, i) => `<article class="work-card"><div class="work-top"><span>[${String(i+1).padStart(2,'0')}]</span><span>${i === 0 ? 'SERVICE DEVELOPMENT' : 'WEBSITE RENEWAL'}</span></div><div class="work-media">${i === 0 ? `<img src="assets/work-${w.src}.png" alt="${escape(w.name)} 웹사이트" loading="lazy">` : `<video muted playsinline poster="assets/work-${w.src}.png" preload="metadata" aria-label="${escape(w.name)} 웹사이트 작업 영상"><source src="assets/work-${w.src}.mp4" type="video/mp4"></video>`}</div><div class="work-copy"><h3>${escape(w.name)}</h3><p class="what">${escape(w.what || '홈페이지 전체 리뉴얼')}</p>${w.more ? (i === 0 ? `<details open><summary>담당 업무</summary><ul>${w.more.split('\n').map(item => `<li>${escape(item)}</li>`).join('')}</ul></details>` : `<p class="work-description">${escape(w.more)}</p>`) : ''}${tags(w.tech)}${w.href ? link(w.href, '사이트 바로가기') : ''}</div></article>`).join('');
document.querySelectorAll('.work-media video').forEach(video => {
  video.parentElement.addEventListener('mouseenter', () => video.play().catch(() => {}));
  video.parentElement.addEventListener('mouseleave', () => { video.pause(); video.load(); });
});
